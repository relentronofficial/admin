...
export async function getMemberHandler(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    request.server.log.error({ err, memberId: (request.params as any)?.id }, 'getMemberHandler failed');
    return reply.status(500).send({ success: false, data: null, error: err?.message || 'Failed to load member' });
  }
}
