    ...
    <div>
      ...
      <div className="flex items-center gap-3">
        <div className="w-16 h-12 rounded-lg border border-[#333] bg-[#141414] flex items-center justify-center overflow-hidden shrink-0">
          {value ? <img src={value} alt="" className="w-full h-full object-cover" /> : <ImageIcon size={16} className="text-[#777]" />}
        </div>
        <div className="space-y-1.5">
          ...
          {value && (
            <button type="button" onClick={() => onUploaded("")} className="flex items-center gap-1 text-[10px] text-[#777] hover:text-red-400 transition-colors">
              <X size={10} /> Clear
            </button>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#888] font-medium uppercase tracking-[1px] font-rajdhani">Manage home feed sections and their content items.</p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl overflow-hidden flex flex-col">
            ...
            {isLoading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-10 gap-2 text-center">
                ...
                <p className="text-[#888] font-rajdhani font-bold uppercase tracking-widest text-xs">No sections yet</p>
              </div>
            ) : (
                ...
                {localSections.map((s: any, i: number) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[#777] truncate font-mono">/{s.slug}</p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      {!s.isVisible && <EyeOff size={11} className="text-[#777]" />}
                      {s.requiredTier > 0 && <span className="text-[9px] text-[#888] bg-[#1a1a1a] border border-[#333] px-1 py-0.5 rounded font-bold font-rajdhani">T{s.requiredTier}</span>}
                      <button onClick={e => { e.stopPropagation(); openEditSection(s); }} className="p-1 text-[#777] hover:text-green-400 rounded transition-all"><Pencil size={11} /></button>
                      <button onClick={e => { e.stopPropagation(); setDeletingSection(s.id); }} className="p-1 text-[#777] hover:text-red-400 rounded transition-all"><Trash2 size={11} /></button>
                    </div>
                    <ChevronRight size={12} className={selectedSection?.id === s.id ? "text-[#dc2626]" : "text-[#666]"} />
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl overflow-hidden flex flex-col">
            {!selectedSection ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
                ...
                <p className="text-[#777] font-rajdhani font-bold uppercase tracking-widest text-sm">Select a section</p>
                <p className="text-[#666] text-xs">Click any section on the left to manage its items.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between p-4 border-b border-[#2a2a2a] bg-[#1a1a1a] shrink-0">
                  <div className="flex items-center gap-2 min-w-0">
                    <div>
                      ...
                      <p className="text-[10px] text-[#777] font-mono">/{selectedSection.slug}</p>
                    </div>
                    {itemsDirty && (
                ...
                {itemsLoading ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
                    <Film size={28} className="text-[#666]" />
                    <p className="text-[#888] font-rajdhani font-bold uppercase tracking-widest text-xs">No items in this section</p>
                  </div>
                ) : (
                  <div className="flex-1 overflow-y-auto divide-y divide-[#222]">
                    {localItems.map((item: any, i: number) => (
                      ...
                      >
                        <GripVertical size={14} className="text-[#666] cursor-grab shrink-0" />
                        {item.thumbnailUrl ? (
                          ...
                          <div className="w-14 h-10 rounded bg-[#1a1a1a] border border-[#333] flex items-center justify-center shrink-0">
                            <Film size={14} className="text-[#666]" />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            ...
                            {item.categoryTag && <span className="text-[10px] text-[#888] flex items-center gap-0.5"><Tag size={9} />{item.categoryTag}</span>}
                            {item.requiredTier > 0 && <span className="text-[10px] text-[#888] bg-[#1a1a1a] border border-[#333] px-1.5 py-0.5 rounded font-rajdhani font-bold">T{item.requiredTier}</span>}
                            ...
                            {!item.isVisible && <EyeOff size={10} className="text-[#777]" />}
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button onClick={() => openEditItem(item)} className="p-1.5 text-[#777] hover:text-green-400 hover:bg-green-400/10 rounded transition-all"><Pencil size={13} /></button>
                          <button onClick={() => setDeletingItem(item.id)} className="p-1.5 text-[#777] hover:text-red-400 hover:bg-red-400/10 rounded transition-all"><Trash2 size={13} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-[#141414] border border-[#2a2a2a] w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
            <div className="px-6 py-5 border-b border-[#2a2a2a] flex justify-between items-center bg-[#1a1a1a]">
              ...
              <button onClick={() => setShowSectionForm(false)} className="text-[#888] hover:text-white"><X size={18} /></button>
            </div>
            ...
            <div className="px-6 py-4 border-t border-[#2a2a2a] bg-[#1a1a1a] flex justify-end gap-3">
              <button onClick={() => setShowSectionForm(false)} className="px-6 py-2 text-[#888] hover:text-white font-rajdhani font-bold text-[12px] uppercase tracking-widest transition-all">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-[#141414] border border-[#2a2a2a] w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl">
            <div className="px-6 py-5 border-b border-[#2a2a2a] flex justify-between items-center bg-[#1a1a1a]">
              ...
              <button onClick={() => setShowItemForm(false)} className="text-[#888] hover:text-white"><X size={18} /></button>
            </div>
            <div className="p-6 space-y-4 max-h-[72vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Content Type</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Category Tag</label>
              ...
              {/* Link Type */}
              <div>
                <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Link Type</label>
              ...
              {itemForm.linkType === "workshop" && (
                <div>
                  <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Workshop *</label>
              ...
              {itemForm.linkType === "course" && (
                <div>
                  <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Course *</label>
              ...
              {itemForm.linkType === "external" && (
                <div>
                  <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">External URL *</label>
            ...
            <div className="px-6 py-4 border-t border-[#2a2a2a] bg-[#1a1a1a] flex justify-end gap-3">
              <button onClick={() => setShowItemForm(false)} className="px-6 py-2 text-[#888] hover:text-white font-rajdhani font-bold text-[12px] uppercase tracking-widest transition-all">Cancel</button>
