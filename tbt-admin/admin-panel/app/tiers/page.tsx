        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#888] font-medium uppercase tracking-[1px] font-rajdhani">Manage access tiers for content gating.</p>
            </div>
          </div>
        ...
        <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl overflow-hidden">
          {isLoading ? (
            ...
            <div className="text-center py-20 space-y-3">
              <Trophy size={36} className="mx-auto text-[#666]" />
              <p className="text-[#888] font-rajdhani font-bold uppercase tracking-widest text-sm">No tiers configured</p>
            </div>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="bg-[#1a1a1a] border-b border-[#2a2a2a]">
                  <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Tier #</th>
                  <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Label</th>
                  <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Description</th>
                  <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Unlock Condition</th>
                  <th className="px-6 py-4 text-left text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Active</th>
              ...
              <tbody className="divide-y divide-[#2a2a2a]">
                {tiers.map((tier: any) => (
                  <tr key={tier.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-6 py-4 text-[#888] text-sm max-w-[180px] truncate">{tier.description || "—"}</td>
                    <td className="px-6 py-4 text-[#888] text-sm">{tier.unlockConditionText || "—"}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest font-rajdhani ${tier.isActive ? "bg-green-500/10 text-green-400" : "bg-[#222] text-[#777]"}`}>
                        {tier.isActive ? "Active" : "Inactive"}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openEdit(tier)} className="p-1.5 text-[#777] hover:text-green-400 hover:bg-green-400/10 rounded transition-all">
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => setDeleting(tier.id)} className="p-1.5 text-[#777] hover:text-red-400 hover:bg-red-400/10 rounded transition-all">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
          ...
          <div className="bg-[#141414] border border-[#2a2a2a] w-full max-w-md rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Tier Number *</label>
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#888] uppercase tracking-widest mb-2 font-rajdhani">Unlock Condition Text</label>
              ...
              <div className="flex items-center justify-between bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-4 h-11">
                <span className="text-[11px] font-bold text-[#888] uppercase tracking-widest font-rajdhani">Active</span>
                <button
                  type="button"
