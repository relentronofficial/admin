    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        {["", "pending", "contacted", "resolved"].map(s => (
          <button
            ...
            className={`px-4 py-1.5 rounded-md font-rajdhani font-bold text-[11px] uppercase tracking-widest transition-all ${statusFilter === s ? "bg-[#dc2626] text-white" : "bg-[#181818] border border-[#2a2a2a] text-[#888] hover:text-white"}`}
          >
      ...
      {isLoading ? (
        ...
        <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl text-center py-16 space-y-2">
          <ShoppingCart size={28} className="mx-auto text-[#666]" />
          <p className="text-[#888] font-rajdhani font-bold uppercase tracking-widest text-xs">No inquiries</p>
        </div>
      ) : (
              ...
              <div key={inq.id} className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-4 flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#a0a0a0] mt-0.5">
                    ...
                    {inq.member?.email && <> · <span className="text-[#888]">{inq.member.email}</span></>}
                  </p>
                  {inq.message && (
                    <p className="text-xs text-[#888] mt-1.5 italic">"{inq.message}"</p>
                  )}
                  <p className="text-[10px] text-[#777] mt-1.5 font-rajdhani">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-3 pt-2">
          ...
          <span className="text-[11px] text-[#777] font-rajdhani">{page} / {totalPages}</span>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#888] font-medium uppercase tracking-[1px] font-rajdhani">Manage TBT product offerings shown to members.</p>
            </div>
          </div>
        ...
        <div className="flex gap-1 bg-[#181818] border border-[#2a2a2a] rounded-xl p-1 w-fit">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 rounded-lg font-rajdhani font-bold text-[12px] uppercase tracking-widest transition-all ${activeTab === "products" ? "bg-[#dc2626] text-white" : "text-[#888] hover:text-white"}`}
          >
            <ShoppingBag size={13} /> Products
          </button>
          <button
            onClick={() => setActiveTab("inquiries")}
            className={`flex items-center gap-2 px-5 py-2 rounded-lg font-rajdhani font-bold text-[12px] uppercase tracking-widest transition-all ${activeTab === "inquiries" ? "bg-[#dc2626] text-white" : "text-[#888] hover:text-white"}`}
          >
        ...
        <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelCls}>Page Background <span className="text-[#666] normal-case font-normal">(CSS gradient or color)</span></label>
        ...
        {isLoading ? (
          ...
          <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl text-center py-20 space-y-3">
            <ShoppingBag size={36} className="mx-auto text-[#666]" />
            <p className="text-[#888] font-rajdhani font-bold uppercase tracking-widest text-sm">No products configured</p>
          </div>
        ) : (
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => handleToggleVisible(p)} className={`p-1.5 rounded transition-all ${p.isVisible ? "text-green-400 hover:bg-green-400/10" : "text-[#777] hover:text-white hover:bg-[#2a2a2a]"}`}>
                        {p.isVisible ? <Eye size={14} /> : <EyeOff size={14} />}
                      </button>
                      <button onClick={() => openEdit(p)} className="p-1.5 text-[#777] hover:text-green-400 hover:bg-green-400/10 rounded transition-all"><Pencil size={14} /></button>
                      <button onClick={() => setDeleting(p.id)} className="p-1.5 text-[#777] hover:text-red-400 hover:bg-red-400/10 rounded transition-all"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  {p.description && <p className="text-[#888] text-xs leading-relaxed mb-3 line-clamp-2">{p.description}</p>}
                  {p.ctas?.length > 0 && (
          ...
          <div className="bg-[#141414] border border-[#2a2a2a] w-full max-w-xl rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4 max-h-[72vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>Price <span className="normal-case font-normal text-[#666]">(leave blank if free)</span></label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
              ...
              <div>
                ...
                {form.thumbnailUrl ? (
                  ...
                  <button type="button" onClick={() => thumbInputRef.current?.click()} disabled={uploadingThumb} className="w-full bg-[#1a1a1a] border border-dashed border-[#333] hover:border-[#dc2626] rounded-lg h-11 px-4 text-[12px] text-[#888] hover:text-white transition-all flex items-center justify-center gap-2">
                ...
                <div className="space-y-3">
                  {form.ctas.map((cta: any, i: number) => (
                    <div key={i} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-[#777] uppercase font-rajdhani font-bold tracking-widest">CTA {i + 1}</span>
                        {form.ctas.length > 1 && (
                          <button type="button" onClick={() => removeCta(i)} className="text-[#777] hover:text-red-400"><X size={12} /></button>
                        )}
                      </div>
