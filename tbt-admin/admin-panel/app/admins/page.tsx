        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#888] font-medium uppercase tracking-[1px] font-rajdhani">Manage system administrators and their operational permissions.</p>
            </div>
          </div>
          <button
          ...
          <div className="p-4 border-b border-[#2a2a2a] flex flex-col md:flex-row gap-4 items-center justify-between bg-[#1a1a1a]/50">
            <div className="relative w-full md:w-[400px]">
              <input
                ...
                className="w-full bg-[#141414] border border-[#333] rounded-md py-2.5 pl-10 pr-4 text-[12px] font-rajdhani font-bold tracking-wider outline-none focus:border-[#e02020] transition-all text-[#f0f0f0] placeholder:text-[#777]"
              />
          ...
          <div className="overflow-x-auto min-h-[400px]">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                <Loader2 size={40} className="animate-spin text-[#e02020]" />
                <p className="text-[11px] text-[#888] font-bold tracking-[2px] uppercase font-rajdhani">Accessing Database...</p>
              </div>
            ) : isError ? (
              <div className="text-center py-20 space-y-4">
                ...
                <p className="text-[12px] text-[#888]">Failed to retrieve admin records. Please try again.</p>
              </div>
            ) : admins.length === 0 ? (
              <div className="text-center py-20 space-y-4">
                <User size={40} className="mx-auto text-[#666]" />
                <p className="text-[#888] font-rajdhani font-bold uppercase tracking-widest">No Records Found</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#1a1a1a] border-b border-[#2a2a2a]">
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Admin Profile</th>
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">System ID</th>
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani text-center">Status</th>
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Privilege</th>
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani">Department</th>
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[2px] text-[#888] font-bold font-rajdhani text-right">Last Login</th>
                    ...
                    <tr key={admin.id} className="group hover:bg-white/[0.02] transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              <p className="text-[11px] text-[#888] truncate max-w-[150px]">{admin.email}</p>
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <p className="text-[10px] text-[#888] uppercase tracking-wider font-rajdhani mt-0.5">{admin.department || "General"}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        ...
                        <p className="text-[9px] text-[#888] uppercase tracking-tighter mt-0.5 font-rajdhani">
                  ...
                  <button key={i} onClick={() => setPage(i + 1)}
                    className={cn("w-8 h-8 rounded text-[11px] font-bold font-rajdhani transition-all",
                      page === i + 1
                        ...
                        : "bg-[#141414] border border-[#333] text-[#888] hover:border-[#606060]"
                    )}>
                    {i + 1}
                  </button>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setStatusTarget(null)} className="text-[#777] hover:text-white transition-colors">
                <X size={18} />
              </button>
            </div>
            <p className="text-[12px] text-[#888] mb-4 font-rajdhani">
                ...
                <button key={opt.value} onClick={() => setNewStatus(opt.value)}
                  className={cn(
                    ...
                    newStatus === opt.value
                      ...
                      : "border-[#333] text-[#888] hover:border-[#606060] hover:text-[#a0a0a0]"
                  )}>
                  {opt.label}
                </button>
              ))}
