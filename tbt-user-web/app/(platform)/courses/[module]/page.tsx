...
import {
  ...
  ShoppingBag, Briefcase, Users, ArrowLeft,
} from "lucide-react";
...
function ContinueLearningCardSkeleton() {
  return (
    <div className="rounded-2xl overflow-hidden animate-pulse flex gap-5 p-5"
      style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)" }}>
...
function EnrolledCardSkeleton() {
  return (
    <div className="rounded-2xl overflow-hidden flex gap-4 p-4 animate-pulse shrink-0 w-72"
      style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)" }}>
...
function ContinueLearningCourseCard({ item }: { item: ContinueLearningItem }) {
  ...
  return (
    <Link href={`/learning/${item.id}?lesson=${item.lessonId}`}
      className="group flex gap-5 p-5 rounded-2xl transition-all duration-200"
      style={{ background: "var(--color-bg-surface)", border: "1px solid color-mix(in srgb, var(--color-accent) 20%, var(--color-border-subtle))" }}
      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.border = "1px solid color-mix(in srgb, var(--color-accent) 45%, transparent)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 4px 24px rgba(0,0,0,0.3)"; }}
      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.border = "1px solid color-mix(in srgb, var(--color-accent) 20%, var(--color-border-subtle))"; (e.currentTarget as HTMLElement).style.boxShadow = "none"; }}
    >
      <div className="relative w-36 h-24 rounded-xl overflow-hidden shrink-0 bg-black">
        {item.thumbnailUrl
          ...
          : <div className="w-full h-full flex items-center justify-center"><BookOpen size={22} className="text-muted-foreground opacity-20" /></div>
        }
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200" style={{ background: "rgba(0,0,0,0.55)" }}>
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "var(--color-accent)" }}>Continue Learning</span>
          <h3 className="text-[15px] font-semibold text-foreground mt-1 line-clamp-1 group-hover:text-[var(--color-accent)] transition-colors">{item.title}</h3>
          {item.lastLessonTitle && (
            <p className="text-[12px] text-muted-foreground mt-1 line-clamp-1 flex items-center gap-1.5">
              <Play size={10} />{item.lastLessonTitle}
            </p>
          )}
        </div>
        <div className="space-y-2 mt-3">
          <div className="flex items-center justify-between text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1.5"><Layers size={10} />{completedLessons} / {item.episodeCount} videos completed · {pct}% done</span>
            {videoWatchedPct > 0 && <span className="flex items-center gap-1 shrink-0 ml-2"><Clock size={9} />{videoWatchedPct}% of current video</span>}
          </div>
    ...
    <Link href={`/learning/${enrollment.courseId}`}
      ...
      <div className="w-20 h-16 rounded-xl overflow-hidden shrink-0 relative bg-black">
        {course?.thumbnailUrl
          ...
          : <div className="w-full h-full flex items-center justify-center"><BookOpen size={18} className="text-muted-foreground opacity-20" /></div>
        }
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <p className="text-[13px] font-semibold text-foreground line-clamp-2 leading-snug group-hover:text-[var(--color-accent)] transition-colors">{course?.title}</p>
        <div className="mt-2">
          {completed ? (
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between gap-1.5">
                ...
                {enrollment.totalLessons > 0 && <span className="text-[11px] text-muted-foreground shrink-0">{enrollment.completedLessons ?? 0}/{enrollment.totalLessons}</span>}
              </div>
...
function CourseCard({ course, isEnrolled, progress }: { course: any; isEnrolled: boolean; progress?: number }) {
  ...
  return (
    <Link href={`/learning/${course.id}`}
      ...
      style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)", boxShadow: "0 2px 12px rgba(0,0,0,0.3)" }}
      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.border = "1px solid color-mix(in srgb, var(--color-accent) 40%, transparent)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 4px 24px rgba(0,0,0,0.4)"; }}
      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.border = "1px solid var(--color-border-subtle)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 2px 12px rgba(0,0,0,0.3)"; }}
    >
      <div className="relative aspect-video overflow-hidden" style={{ background: "var(--color-surface-overlay-xs)" }}>
        {course.thumbnailUrl
          ...
          : <div className="absolute inset-0 flex items-center justify-center"><BookOpen size={36} className="text-muted-foreground opacity-10" /></div>
        }
        <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.6) 0%, transparent 50%)" }} />
        <div className="absolute top-3 left-3">
          <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-lg backdrop-blur-sm"
            style={{ background: "color-mix(in srgb, var(--color-accent) 20%, rgba(0,0,0,0.7))", border: "1px solid color-mix(in srgb, var(--color-accent) 35%, transparent)", color: "var(--color-accent)" }}>
            {course.level || "course"}
          </span>
        </div>
        <div className="absolute top-3 right-3">
          {isEnrolled ? (
            <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg backdrop-blur-sm"
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        <div className="flex-1">
          <h3 className="font-semibold text-[14px] leading-snug text-foreground line-clamp-2 group-hover:text-[var(--color-accent)] transition-colors duration-150">{course.title}</h3>
          {course.description && <p className="text-[12px] text-muted-foreground mt-1.5 line-clamp-2 leading-relaxed">{course.description}</p>}
        </div>
        <div className="flex items-center gap-3 text-[11px] text-muted-foreground border-t pt-3" style={{ borderColor: "var(--color-border-subtle)" }}>
          {course._count?.lessons != null && <span className="flex items-center gap-1"><Layers size={11} /> {course._count.lessons} episodes</span>}
          {course.durationDisplay && <span className="flex items-center gap-1"><Clock size={11} /> {course.durationDisplay}</span>}
          {course.xpPerEpisode && <span className="flex items-center gap-1 ml-auto" style={{ color: "var(--color-accent)" }}><Zap size={11} /> {course.xpPerEpisode} XP/ep</span>}
        </div>
        <div className="flex items-center justify-between px-3 py-2.5 rounded-xl text-[12px] font-semibold transition-all duration-200"
          style={{
            background: isEnrolled ? "color-mix(in srgb, var(--color-accent) 12%, var(--color-surface-overlay))" : "var(--color-surface-overlay)",
            border: isEnrolled ? "1px solid color-mix(in srgb, var(--color-accent) 25%, transparent)" : "1px solid var(--color-border-subtle)",
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="relative overflow-hidden rounded-2xl px-6 py-8 md:px-10 md:py-10"
        style={{ background: "color-mix(in srgb, var(--color-accent) 8%, var(--color-bg-surface, #181818))", border: "1px solid color-mix(in srgb, var(--color-accent) 20%, transparent)" }}>
        <div className="absolute -top-16 -right-16 w-64 h-64 rounded-full blur-3xl pointer-events-none"
          style={{ background: "color-mix(in srgb, var(--color-accent) 12%, transparent)" }} />
        <div className="relative flex items-start justify-between gap-4">
          <div>
            {/* Back link */}
            <Link href="/courses" className="inline-flex items-center gap-1.5 text-[11px] font-semibold mb-3 transition-colors"
            ...
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                style={{ background: "color-mix(in srgb, var(--color-accent) 18%, transparent)", border: "1px solid color-mix(in srgb, var(--color-accent) 30%, transparent)", color: "var(--color-accent)" }}>
                {cfg.icon}
              </div>
              <div>
          ...
          <div className="hidden md:flex items-center gap-3 shrink-0">
            <Link href="/learning/badges"
              ...
              style={{ background: "var(--color-surface-overlay-md)", border: "1px solid var(--color-border-medium)", color: "var(--color-text-secondary)" }}>
      ...
      <section className="space-y-4">
        <h2 className="text-base font-bold text-foreground flex items-center gap-2">
          <Play size={15} style={{ color: "var(--color-accent)" }} /> Continue Learning
        </h2>
        {continueLoading ? (
          ...
          <div className="flex items-center gap-4 px-5 py-4 rounded-2xl"
            style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)" }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: "var(--color-surface-overlay)" }}>
            ...
            <div>
              ...
              <p className="text-[12px] text-muted-foreground mt-0.5">Enroll in a course below and start learning.</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {continueCourseItems.slice(0, 3).map((item) => <ContinueLearningCourseCard key={item.id} item={item} />)}
          </div>
        )}
      </section>
      ...
      {(enrollLoading || myEnrollments.length > 0) && (
        <section className="space-y-4">
          <h2 className="text-base font-bold text-foreground flex items-center gap-2">
            <BookOpen size={15} style={{ color: "var(--color-accent)" }} /> Enrolled Courses
          </h2>
      ...
      <section className="space-y-5">
        ...
        {catalogLoading ? (
          ...
          <div className="flex flex-col items-center py-20 rounded-2xl gap-3"
            style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border-subtle)" }}>
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: "var(--color-surface-overlay)" }}>
            ...
            {(search || level !== "all" || category !== "all") && (
              <button onClick={() => { setSearch(""); setLevel("all"); setCategory("all"); }}
                className="text-[12px] font-semibold" style={{ color: "var(--color-accent)" }}>
                Clear filters
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {catalogCourses.map((course: any) => {
              ...
              return <CourseCard key={course.id} course={course} isEnrolled={!!enrollment} progress={enrollment?.progressPercent} />;
            })}
          </div>
        )}
      </section>
