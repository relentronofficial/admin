...
function CourseCardSkeleton() {
  return (
    <div
      ...
      style={{ background: "var(--color-bg-surface, #181818)", border: "1px solid rgba(255,255,255,0.06)" }}
    >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="h-4 rounded-full w-4/5" style={{ background: "rgba(255,255,255,0.06)" }} />
        <div className="h-3 rounded-full w-3/5" style={{ background: "rgba(255,255,255,0.04)" }} />
        <div className="h-8 rounded-xl mt-4" style={{ background: "rgba(255,255,255,0.04)" }} />
      </div>
    </div>
  );
}
...
function EnrolledCardSkeleton() {
  return (
    <div
      ...
      style={{ background: "var(--color-bg-surface, #181818)", border: "1px solid rgba(255,255,255,0.06)" }}
    >
      <div className="w-20 h-16 rounded-xl shrink-0" style={{ background: "rgba(255,255,255,0.06)" }} />
      <div className="flex-1 space-y-2 pt-1">
        <div className="h-3.5 rounded-full w-4/5" style={{ background: "rgba(255,255,255,0.06)" }} />
        <div className="h-2.5 rounded-full w-3/5" style={{ background: "rgba(255,255,255,0.04)" }} />
        <div className="h-2 rounded-full w-full mt-3" style={{ background: "rgba(255,255,255,0.04)" }} />
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          <p className="text-[13px] font-semibold text-white line-clamp-2 leading-snug group-hover:text-[var(--color-accent)] transition-colors">
            {course?.title}
          </p>
        </div>
        <div className="mt-2">
          {completed ? (
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-white/40">{pct}% complete</span>
              </div>
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        <div className="flex-1">
          <h3 className="font-semibold text-[14px] leading-snug text-white line-clamp-2 group-hover:text-[var(--color-accent)] transition-colors duration-150">
            {course.title}
          </h3>
          {course.description && (
            <p className="text-[12px] text-white/40 mt-1.5 line-clamp-2 leading-relaxed">
              {course.description}
            </p>
          )}
        </div>
        ...
        {/* Stats row */}
        <div className="flex items-center gap-3 text-[11px] text-white/40 border-t pt-3" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        <div
          ...
          style={{
            background: isEnrolled
              ? "color-mix(in srgb, var(--color-accent) 12%, rgba(255,255,255,0.04))"
              : "rgba(255,255,255,0.04)",
            border: isEnrolled
              ? "1px solid color-mix(in srgb, var(--color-accent) 25%, transparent)"
              : "1px solid rgba(255,255,255,0.07)",
            color: isEnrolled ? "var(--color-accent)" : "rgba(255,255,255,0.6)",
          }}
        >
      ...
      >
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white leading-tight">Courses</h1>
        ...
        {myEnrollments.length > 0 && (
          <div className="flex items-center gap-6 mt-6 pt-5" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            {[
      ...
      <section className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          {!catalogLoading && catalogCourses.length > 0 && (
            <span className="text-[11px] text-white/30">{catalogCourses.length} courses</span>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 h-10 text-sm text-white placeholder:text-white/30 rounded-xl outline-none transition-all"
              style={{
                background: "var(--color-bg-surface, #181818)",
        ...
        {catalogLoading ? (
          ...
          >
            ...
            <p className="text-white/40 text-sm font-medium">No courses found</p>
            {(search || level !== "all") && (
              <button
