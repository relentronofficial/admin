...
import {
  Volume2, VolumeX, ChevronLeft, ChevronRight, Play, Lock,
  PlayCircle, CheckCircle2, Clock, X, ArrowRight,
} from "lucide-react";
...
function ContentItemCard({ item }: { item: ContentItem }) {
      ...
      <div className="aspect-[2/3] md:aspect-video relative overflow-hidden">
        {item.thumbnailUrl ? (
          <Image
            ...
            sizes="(max-width: 768px) 176px, 29vw"
          />
        ) : (
  ...
  return (
    <div
      className={cn(
        "flex-shrink-0 w-44 md:w-[29vw] lg:w-[21vw]",
...
export function TbtSections({ sections }: { sections: ContentSection[] }) {
      ...
      <div className="flex gap-3 p-3">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-0.5 py-0.5">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <span className="text-[10px] text-muted-foreground">{relativeTimeFromEpoch(item.updatedAt)}</span>
          </div>
        </div>
      </div>
...
// ─── Tier-aware sections refetch ─────────────────────────────────────────────
// After hydration, if the user's tier is higher than the default (1) used for
// SSR, silently refetch sections so locked/unlocked state is accurate.

export function TierAwareSectionsRefetch({
  onSections,
}: {
  onSections: (sections: ContentSection[]) => void;
}) {
  const { data: me } = useMe();

  useEffect(() => {
    const tier = me?.currentTier ?? 1;
    if (tier <= 1) return; // SSR data is already correct for tier 1

    let cancelled = false;
    apiClient
      .get(`/api/user/home/sections?memberTier=${tier}`)
      .then((res: any) => {
        if (!cancelled && res?.data?.sections) {
          onSections(res.data.sections);
        }
      })
      .catch(() => {});

    return () => { cancelled = true; };
  }, [me?.currentTier]); // eslint-disable-line react-hooks/exhaustive-deps
